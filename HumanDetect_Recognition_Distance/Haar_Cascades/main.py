# Resources
# https://www.youtube.com/watch?v=DfVsXMkv8cA : to install the imports

# https://www.youtube.com/watch?v=V62M9d8QkYM&t=62s : the opencv code
# Note the link above, detectCommonObjects doesn't work

# https://www.youtube.com/watch?v=88HdqNDQsEk : using the Haar Cascade
# Note the link above works and can detect faces

# Imports
import cv2
import time
detectionModel = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')

# Import of coordinates data
#import sys
#sys.path.append("/media/jetbot/jetbot2024/Derick_Folder")
import JetsonCords

def gstreamer_pipeline(
    sensor_id=0,
    capture_width=1920,
    capture_height=1080,
    display_width=960,
    display_height=540,
    framerate=30,
    flip_method=0,
):
    return (
        "nvarguscamerasrc sensor-id=%d ! "
        "video/x-raw(memory:NVMM), width=(int)%d, height=(int)%d, framerate=(fraction)%d/1 ! "
        "nvvidconv flip-method=%d ! "
        "video/x-raw, width=(int)%d, height=(int)%d, format=(string)BGRx ! "
        "videoconvert ! "
        "video/x-raw, format=(string)BGR ! appsink"
        % (
            sensor_id,
            capture_width,
            capture_height,
            framerate,
            flip_method,
            display_width,
            display_height,
        )
    )

#Obtain video data
video = cv2.VideoCapture(gstreamer_pipeline(flip_method=0), cv2.CAP_GSTREAMER)

# Initialize nodes for coordinates data
Position = JetsonCords.RobotPos()

while True:
    ret, frame = video.read()
    # Convert frame to grayscale for faster detection
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    # Detect faces using haar cascade file
    detect = detectionModel.detectMultiScale(gray, 1.3, 5)

    # Identify faces using a rectangle
    for (x,y,w,h) in detect:
        cv2.rectangle(frame, (x,y), (x+w, y+h), (0, 255, 255), 5)
        cv2.putText(frame, 'Human Identified!', (x+w,y), cv2.FONT_ITALIC, 1, (255, 255, 255), 2)
        
	# Get position after human has been detected
        pos = Position.getCords()
        

    # Display the video
    cv2.imshow("object", frame)


    # press q to exit program
    if cv2.waitKey(1) & 0xFF == ord("q"):
        break

video.release()
cv2.destroyAllWindows()
Position.shutNodes()
